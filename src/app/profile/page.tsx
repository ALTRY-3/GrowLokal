...
const ORDER_DATE_FORMATTER = new Intl.DateTimeFormat("en-PH", {
  month: "short",
  day: "numeric",
  year: "numeric",
});

const ORDER_ITEM_PLACEHOLDER = "https://placehold.co/80x80?text=No+Image";

const formatCurrency = (value?: number) => PESO_FORMATTER.format(value ?? 0);
...
const resolveProductId = (value: ProfileOrderItem["productId"]) => {
  if (!value) return "";
...
export default function ProfilePage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      const itemMatch = order.items?.some((item) => {
        ...
        const sellerMatch = item?.artistName?.toLowerCase?.().includes(query);
        return Boolean(nameMatch || sellerMatch);
      });
  ...
  }, [session?.user, activeSection, activeOrdersTab]);

  const consumeOrdersRefreshFlag = useCallback(() => {
    if (typeof window === "undefined" || activeSection !== "orders") {
      return;
    }

    const pendingRefresh = sessionStorage.getItem("ordersPendingRefresh");
    if (pendingRefresh) {
      sessionStorage.removeItem("ordersPendingRefresh");
      fetchOrders();
    }
                ...
                <div className="orders-content">
                  {!isLoadingOrders &&
                    ...
                    filteredOrders.map((order) => {
                      ...
                      const paymentMethodLabel = order.paymentDetails?.method
                        ? order.paymentDetails.method.toUpperCase()
                        : "—";
                      const paymentStatusLabel = order.paymentDetails?.status
                        ? order.paymentDetails.status.replace(/_/g, " ")
                        : "pending";
                      const statusClass = `order-status-pill ${
                      ...
                      return (
                        <div className="order-card" key={orderKey}>
                          ...
                          {orderItems.map((item, index) => {
                            const itemKey = getOrderItemKey(order, item, index);
                            const isLastItem = index === orderItems.length - 1;
                            const bodyClass = `order-body${
                              isLastItem ? " order-body-last" : ""
                            }`;
                            const lineTotal =
                              (item?.price || 0) * (item?.quantity || 1);

                            return (
                              <div className={bodyClass} key={itemKey}>
                                <img
                                ...
                                <div className="order-details">
                                  ...
                                  <div className="order-artist">
                                    {item?.artistName || "Artisan"}
                                  </div>
